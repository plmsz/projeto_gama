...
import ProfilePatient from './ProfilePatient/index'

export function Anamnesis() {
  const { userId } = useParams()
  const { data: dataProfile } = useFetch(`users?userId=${userId}`)
  const { data } = useFetch(`anamnesis?userId=${userId}`)
  if (data) {
    const initialValues = {
  ...
  //TODO: editar, css do tabs, modifica Profile, sentimentos coM  select multiplo
  return (
    <>
      <Tabs.Root defaultValue='tab1'>
            ...
            <Grid container spacing={1} mt={2} mb={2}>
              ...
              <Grid item xs={24} sm={4} md={3}>
                <InputLabel
                  id=''
                  component='legend'
                  error={errors.reasonMentalHealthCare && watchTypeHealthCare === 'Sim'}
                >
              ...
              <Grid item xs={24} sm={4} md={3}>
                ...
                />
                <FormHelperText error={true}>
                  {errors.reasonMedicine?.message && watchMedicine === 'Sim'}
                </FormHelperText>
              </Grid>
              <Grid item xs={24} sm={4} md={3}>
        ...
        <Tabs.Content value='tab2'>
          <ProfilePatient data={dataProfile[0]} />
        </Tabs.Content>
      </Tabs.Root>
    </>
  )
